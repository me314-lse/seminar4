...
```{r}
car_slope <- coef(______)
_______
car_intercept <- coef(______)
_______
...
```{r}
coef(car_model)[["(Intercept)"]] + coef(car_model)[["wt"]] * 7
...
scatterplot +
  ...
  geom_abline(slope = -5.344, intercept = 37.285, color = "red", linewidth = 1)
```
...
summary(immat_model_po <- lm(_______ ~ _______, data = residuals_po))
