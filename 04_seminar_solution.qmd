...
scatterplot_guess <- scatterplot +
    geom_abline(intercept = mean(mtcars$mpg), slope = 0, color = "red", linewidth = 1)
```

Once you are happy with your guess, let's see well you did! Use the `geom_smooth()` function to add a regression line to the plot. 
...
scatterplot +
    ...
    annotate("point", x = 0, y = car_intercept, color = "black", size = 8, shape = 18)
```
...
scatterplot +
    geom_point(
        ...
        aes(x = wt, y = mpg, color = car), size = 3
    ) +
    geom_text(
        data = newdata_cars_long,
...
```{r}
# set the working directory to the folder where this file and the data are located)
getwd() # this is your current working directory, if the file is not located in that folder, you need to specify the correct path
path <- ("/Users/anton/Library/CloudStorage/OneDrive-LondonSchoolofEconomics/Teaching/2025/ME314_PREP/labs/session_4_ols/data")

ess_data <- readRDS(paste0(path, "/ESS9_immatt.rds"))
...
scatterplot +
    geom_linerange(
        data = residual_df, aes(x = wt, ymin = predicted, ymax = predicted + residuals),
...
summary(immat_model_po <- lm(immatt_residuals ~ hhnetinc_residuals, data = ess_data))
...
modelsummary::modelsummary(
    ...
    coef_rename = c("hhnetinc" = "Household income (decile)", "educ" = "Education (years)", "hhnetinc_residuals" = "Household income (decile, residuals)"),
    stars = T,
...
print(f"Weight coefficient: {model.params['wt']:.3f}")
...
model_multi = smf.ols('mpg ~ wt + hp', data=mtcars).fit()
...
ggplot() +
    ...
    annotate("text",
        x = mean(resampled_means), y = 2, label = "Mean of Means", angle = -90,
        vjust = -1
    ) +
