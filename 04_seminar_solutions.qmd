...
```{r mtcars_explore}
utils::head(mtcars) #  print the first observations of as many variables as fit in the console
summary(mtcars) # print descriptive statistics for each variable
dplyr::glimpse(mtcars) # print first few observations for all variables
```
...
::: {.callout-info}
If any plot elements look too small, Positron's **Plots** pane lets you enlarge a plot or open it in a separate window, so no extra setup is needed.
:::

Ok, we can see that there is a negative relationship between car weight and miles per gallon. Just from looking at the scatterplot, what do you think are appropriate values for the intercept and the slope of the regression line? Try out a few values and add them to the scatterplot by changing respective values in the `geom_abline()` function below.
...
Once you are happy with your guess, let's see how well you did! Use the `geom_smooth()` function to add a straight regression line to the plot.
...
Under the hood, the `geom_smooth()` function estimates a model and then uses its result to fit a line to the data. `method = "lm"` means that the function estimates a "**l**inear **m**odel," which is what we are now going to do manually.
...
```{r estimate_ols}
car_model <- lm(formula = mpg ~ wt, data = mtcars)
car_model
```
...
## Answers
The expected value of Y at x = 0 is the intercept of the regression line.
The output shows that the intercept is 37.285 and the coefficient for weight is -5.344. This means that for every additional 1000 lbs of weight, the miles per gallon decrease by about 5.34.
:::
...
1. Not really! A car will never weigh 0 lbs (and if it does, we can be certain it doesn't run on petrol).
2. The fit doesn't look too bad here, but at the extremes of the weight distribution, we seem to underestimate the dependent variable. There might be something going on with the functional form of the relationship.
...
![Gorbachev's 7000 lbs Limousine](figs/gorbi_car.jpg){fig-align="center" width="400"}
...
1. About -0.12 miles per gallon:
$$
\begin{align}
\hat{y} =& \beta_0 + \beta_1 \times 7 \\
\hat{y} =& 37.285 + (-5.344) \times 7 \\
\hat{y} =& 37.285 - 5.344 \times 7 \\
\hat{y} =& 37.285 - 37.408 \\
\hat{y} \approx& -0.12 \\
\end{align}
...
2. `mpg` values below zero make zero sense! Thinking about the coefficient values in the context of the underlying data generating process is important and helps avoiding misinterpretations. In our case, the effect of weight will probably decrease more and more as we build heavier and heavier cars. As `mpg` is calculated as $\frac{\text{miles}}{\text{gallons}}$, it is also mathematically impossible for this variable to be negative (a car can't move negatively or have a negative usage of gas). It seems like our model is not well suited to predict the consumption of heavy state cars. It seems like the cars we used to estimate our model represent only a subset of existing cars and we can't use this model to predict the consumption of very exotic cars.
:::
...
```{r}
predict(car_model, newdata = data.frame(wt = .13))
```
...
scatterplot +
  ...
  geom_smooth(
    method = "lm",
    formula = y ~ x + I(x^2), # adapt the functional form
    se = FALSE,
    color = "red"
  )
```
...
```{r}
car_model_poly <- lm(formula = mpg ~ wt + I(wt^2), data = mtcars)
...
# predict the values
prediction_poly <- predict(car_model_poly, newdata = newdata_cars)

# we can add the predictions to the newdata_cars dataframe
newdata_cars$mpg_hat <- prediction_poly

# inspect the result
newdata_cars
```

If you have the time, add the line predicted by car_model_poly, your predictions for P50 and ZIL-41047 and the true values to the scatterplot.  You could use the `annotate()` function or pass the predictions to an additional `geom_point()` layer.
```{r}

newdata_cars_long <- newdata_cars %>%
...
scatterplot +
  geom_point(
    data = newdata_cars_long,
    aes(x = wt, y = mpg, color = car),
    size = 3
  ) +
  geom_text(
    data = newdata_cars_long,
    aes(x = wt, y = mpg, label = type, color = car),
    vjust = -1,
    show.legend = FALSE
  ) +
  geom_smooth(
  ...
  scale_color_manual(
    values = c("P50" = "forestgreen", "ZIL-41047" = "red"),
    name = "Car"
  )
```
...
```{r }
confint(car_model, level = 0.999)
confint(car_model, level = 1)
```
...
```{r}
ess_data <- readRDS("data/ESS9_immatt.rds")

dplyr::glimpse(ess_data)
```
...
```{r summarise_ess}
summary(ess_data[, c("immatt", "educ", "hhnetinc")])

hist(ess_data$hhnetinc, main = "Histogram of Household Income Deciles")
hist(ess_data$educ, main = "Histogram of Education (Years)")
hist(ess_data$immatt, main = "Histogram of Immigration Attitudes")
```
...
summary(immatt_model_biv <- lm(immatt ~ hhnetinc, data = ess_data))
summary(immatt_model_multi <- lm(immatt ~ hhnetinc + educ, data = ess_data))
```
...
::: {.callout-tip collapse="true"}
## Answers
1.  The intercept is the expected value of `immatt` when `hhnetinc` is zero. If you inspect the distribution of `hhnetinc`, you will see that this is not a meaningful value, as the lowest decile is 1. So the intercept is not interpretable.

```{r}
table(ess_data$hhnetinc) # check the values of hhnetinc
```

2.  The intercept changes because the multivariate model controls for `educ`, which means that the intercept is now the expected value of `immatt` when both `hhnetinc` and `educ` are zero. Again, this is not a meaningful value, as the lowest value of `educ` is 0 (no education) and the lowest value of `hhnetinc` is 1 (lowest decile).

3.  The coefficient for `hhnetinc` in the bivariate model is the expected change in `immatt` for a one unit increase in `hhnetinc`, while the coefficient in the multivariate model is the expected change in `immatt` for a one unit increase in `hhnetinc`, controlling for `educ`. In other words, the coefficient in the multivariate model is the partial effect of `hhnetinc` on `immatt`, holding `educ` constant.

4.  The p-value of the coefficient for `educ` in the multivariate model is very small, which means that we can reject the null hypothesis that the coefficient is zero. In other words, there is a statistically significant association between `educ` and `immatt`, controlling for `hhnetinc`.

5. Think about what determines income. Education is an obvious factor! People with more education tend to have higher earnings. At the same time, we know that education has an effect of immigration attitudes too: more educated individuals tend to have more positive attitudes. When we don't control for education, the effect of education is absorbed by the coefficient for `hhnetinc`, which is why it is larger in the bivariate model. When we control for education, we are able to isolate the two effects. Because the relationship between education and income is positive, the bivariate model overestimates the effect of income and once we correct for that, the income coefficient decreases.
...
# first define a prediction grid where each row is an observation for which to predict a value
newdata_grid <- data.frame(hhnetinc = c(7, 10, 7), educ = c(15, 2, 22))

# then calculate the predictions
predictions_multi <- predict(immatt_model_multi, newdata = newdata_grid)
predictions_multi
```
...
The second prediction is a bit odd, there will be very few people with only 2 years of education in the 10th income decile. The third prediction on the other hand predicts the education of a person with 22 years of education, which is a lot! As a matter of fact, there are no individuals with that many years of education in the dataset. So be careful, this is an extrapolation.

```{r}
range(ess_data$educ, na.rm = TRUE)
```

The first prediction is the most reasonable one, 15 years of education and 7th decile income is a common combination.
:::
...
By inspecting the residuals, we can learn something about assumptions 1, 2, and 3. You can access the residuals of a model using the `residuals()` function. Let's visualise them in the scatter plot to remind us what they are.
...
scatterplot +
  geom_linerange(
    data = residual_df,
    aes(x = wt, ymin = predicted, ymax = predicted + residuals),
    color = "red",
    alpha = 0.5,
    linetype = 2
  ) +
...
mean(residual_df$residuals) # should be close to zero
```
...
immatt_residuals <- resid(lm(immatt ~ educ, data = ess_data))

# obtain residuals from bivariate regression of hhnetinc on educ
hhnetinc_residuals <- resid(lm(hhnetinc ~ educ, data = ess_data))

# store the residuals in a dataframe so that the two vectors are aligned
residuals_po <- data.frame(
  immatt_residuals = immatt_residuals,
  hhnetinc_residuals = hhnetinc_residuals
)

# estimate relationship between residuals
summary(
  immat_model_po <- lm(
    immatt_residuals ~ hhnetinc_residuals,
    data = residuals_po
  )
)
...
Lets compare this model with the two models we estimated above. A neat way to do this is to use the `modelsummary` package, which creates nice tables with the results of several models, one in each column.

```{r}
modelsummary::modelsummary(
  list(
    bivariate = immatt_model_biv,
    multivariate = immatt_model_multi,
    multivariate_partialing_out = immat_model_po
  ),
  coef_rename = c(
...
# Part 5: Regression Output using broom
R has a range of packages for working with regression output. For today, we will use the {broom} package, which allows us to turn our model output into tidy dataframes.

```{r}
# get the coefficient block of our model as a dataframe
broom::tidy(car_model)

# get the model-level statistics as a dataframe
...
```{r}
# Multivariate regression is just as easy
car_model_multi <- lm(mpg ~ wt + hp, data = mtcars)
broom::tidy(car_model_multi)
broom::glance(car_model_multi)
```
...
print(paste0("Estimated SE from one sample: ", round(researcher_se, 5)))
print(paste0(
  "Estimated SE from ",
  N,
  " resamples: ",
  round(sd_of_resampled_means, 5)
))
...
To get a better understanding of what the code above does, lets look at the probability density function of a t-distribution for 30 degrees of freedom (in the range from -15 to 15).[^1]
...
t_plot +
  geom_vline(
    aes(xintercept = -abs(car_model_wt_t)),
    linetype = 2,
    color = "red"
  )
...
Luckily, that does not mean that we have to give up on ols or inference in such cases. There are two common approaches to deal with heteroscedasticity: robust standard errors and bootstrapped standard errors. The simulations below demonstrate that these approaches are robust to heteroscedasticity, while the asymptotic standard errors are not.
...
# Usually, you would use a package like {boot} for this, but below you can see what happens under the hood.
...
- judge how well a model fits the data by interpreting the $R^2$,
